...
oraz linie „Kicker:”, „Meta:”, „Caption:” / „Podpis:” to treść dla odwiedzających.
...
Kicker: PC · Unity 6 URP 2D · team of 4 · engineering thesis, 2026

Meta: one text for `description`, `og:description` and `twitter:description`: “Systems built for Grave, a 2D top-down survival horror made by a team of four in Unity 6: A* pathfinding, a light mask rendered by a second camera and custom shaders, a seeded dungeon generator with 48 unit tests, and component-based enemy AI.”
...
Materiał: nagranie z gry od właściciela (7,5 s, 4:3), sterowane przewijaniem: 112 klatek w `images/grave/visibility-frames/` (15 kl./s, 800×600), poster `visibility-poster.webp`. Na desktopie klatka stoi obok (sticky), a opis systemu przewija się po drugiej stronie; przewijanie w dół odtwarza nagranie do przodu, w górę do tyłu. Na telefonie klatka przykleja się u góry, a opis przewija się pod nią. Gracz obraca się w pomieszczeniu z filarami, stożek widzenia jest ucinany przez filary, przeciwnik pojawia się tylko w świetle. Lampy i łączenia dwóch świateł nie ma w nagraniu, więc podpis ich nie wspomina.
...
Kicker: PC · Unity 6 URP 2D · zespół 4 osób · praca inżynierska, 2026

Meta: jeden tekst dla `description`, `og:description` i `twitter:description`: „Systemy zbudowane do gry Grave, survival horroru 2D z widokiem z góry, zrobionego w 4-osobowym zespole w Unity 6: pathfinding A*, maska światła renderowana przez drugą kamerę i własne shadery, generator lochów oparty na seedzie, pokryty 48 testami jednostkowymi, oraz AI przeciwników złożone z komponentów.”
...
**Co to pokazuje:** Kompozycję zamiast dziedziczenia w praktyce. „Ślepy” typ przeciwnika, `BlindListenerEnemy`, to podklasa, której jedyną realną treścią jest `[RequireComponent(typeof(SoundPlayerDetector))]`: ślepota oznacza po prostu, że jego prefab nie ma `VisionPlayerDetector`. Ten typ doszedł bez żadnych zmian w `EnemyBase`. Do tego budżetowanie kosztu AI na dużym poziomie bez pogorszenia reakcji przeciwników w pobliżu gracza.

**Kod:** [`Assets/Scripts/Enemy`](https://github.com/dbarans/engineering-project/tree/main/Assets/Scripts/Enemy)

Materiał: diagram w dwóch częściach; strefy odległości niesie diagram, proza ich nie powtarza. (1) Hub ze szprychami, nie pipeline: w środku [`EnemyBase`: state machine / `EnemyBase`: maszyna stanów], strzałki od środka do każdego komponentu (to `EnemyBase` odpytuje komponenty przez interfejsy). Szprychy: `IPlayerDetector` → `VisionPlayerDetector` [Detection: sight / Wykrycie: wzrok]; `INoiseSensor` → `SoundPlayerDetector`, zasilany z `NoiseEvents` [Hearing: suspicion only / Słuch: tylko podejrzenie]; `IMovementStrategy` → `PathfindingMovement` (A*) albo `SimpleDirectMovement` [Movement: A* or direct / Ruch: A* lub bezpośredni]. Dwie pierwsze szprychy można zgrupować jako [Senses / Zmysły]; ruch stoi osobno, to nie zmysł. Obok mały wariant: ten sam hub bez szprychy wzroku [Blind type: no sight / Typ ślepy: bez wzroku]. (2) Koncentryczne pierścienie wokół gracza, od środka: [Every frame / Co klatkę] z dopiskiem [sensor range + margin / zasięg zmysłów + margines]; [Timed, with jitter / Co interwał, z rozrzutem]; [Parked, path released / Uśpiony, ścieżka zwolniona]. Na granicy uśpienia: [Safety floor / Dolny próg bezpieczeństwa] z dopiskiem [never within sensing range / nigdy w zasięgu zmysłów] oraz dwie bliskie linie [Hysteresis: park / wake / Histereza: uśpienie / wybudzenie] (linia wybudzenia nieco wewnątrz linii uśpienia). W prozie i podpisie PL używać tych samych słów co na diagramie: „dolny próg bezpieczeństwa”, „uśpiony” (nie „podłoga”, nie „zaparkowany”).

Podpis (diagram stref): Dolny próg bezpieczeństwa sprawia, że przeciwnik, który mógłby jeszcze wyczuć gracza, nigdy nie zostaje uśpiony; histereza nie pozwala przeciwnikom na granicy przełączać się tam i z powrotem.
...
Materiał: Aria-labels (grave.html), EN → PL. Teksty EN wzięte z HTML, bez zmian w EN.

- Nawigacja „Next case study” → „Następne studium przypadku”
- System 1, blok kodu: „Code excerpt from AStarPathfinder.cs, the FindPath method: region rejection, the heap search loop, the node budget and the partial-path fallback” → „Fragment kodu z AStarPathfinder.cs, metoda FindPath: odrzucenie celu po regionie, pętla wyszukiwania na kopcu, limit węzłów i awaryjny zwrot częściowej ścieżki”
- System 2, diagram: „A four-step pipeline, top to bottom. One: a ray fan per light finds edges and builds a mesh, via OcclusionMeshBuilder, shared by the player and lamps. Two: a mask camera renders those meshes into a RenderTexture with max blending so the brighter light wins, on the VisionMask layer. Three: the result is published as the global _VisionMask texture. Four: a darkness overlay and sprite shaders sample that texture to dim the scene and hide whatever no light reaches.” → „Czteroetapowy pipeline, od góry do dołu. Pierwszy: wachlarz promieni dla każdego światła znajduje krawędzie i buduje mesh za pomocą OcclusionMeshBuilder, wspólnego dla gracza i lamp. Drugi: kamera maski renderuje te meshe do RenderTexture z blendingiem typu max, więc wygrywa jaśniejsze światło; działa na warstwie VisionMask. Trzeci: wynik jest udostępniany jako globalna tekstura _VisionMask. Czwarty: nakładka ciemności i shadery sprite'ów próbkują tę teksturę, żeby przyciemnić scenę i ukryć wszystko, do czego nie dociera światło.”
- System 3, blok kodu: „Code excerpt from RoomCorridorGenerator.cs, the BuildOnce method: the full generation pipeline, one stage per line, each with its own random stream” → „Fragment kodu z RoomCorridorGenerator.cs, metoda BuildOnce: cały pipeline generowania, jeden etap na linię, każdy z własnym strumieniem losowości”
- System 4, diagram hubu: „EnemyBase sits at the centre of a hub. It queries two sense components through interfaces, each connecting to an exact point on the hub's edge: IPlayerDetector, implemented by VisionPlayerDetector for sight, and INoiseSensor, implemented by SoundPlayerDetector and fed by NoiseEvents, for hearing, which only raises suspicion, not detection. A separate, dashed spoke leads down to IMovementStrategy, implemented by PathfindingMovement or SimpleDirectMovement — movement is not a sense. A panel to the side, linked to the hub by its own connector, shows the same hub without the sight spoke: the blind enemy type.” → „EnemyBase stoi w środku huba. Przez interfejsy odpytuje dwa komponenty zmysłów, z których każdy łączy się z konkretnym punktem na krawędzi huba: IPlayerDetector, implementowany przez VisionPlayerDetector, odpowiada za wzrok, a INoiseSensor, implementowany przez SoundPlayerDetector i zasilany z NoiseEvents, za słuch, który wzbudza tylko podejrzenie, a nie wykrycie. Osobna, przerywana szprycha prowadzi w dół do IMovementStrategy, implementowanego przez PathfindingMovement albo SimpleDirectMovement — ruch nie jest zmysłem. Panel z boku, połączony z hubem własną linią, pokazuje ten sam hub bez szprychy wzroku: ślepy typ przeciwnika.”
- System 4, diagram stref, wersja szeroka (`cs-rings-wide`): „Concentric rings around the player, with a legend beside them. The innermost disk is updated every frame, sized to sensor range plus a margin. A middle band updates on a timer with random jitter. The outer band is parked, releasing its cached path. Two close rings mark hysteresis at the parking boundary — a wake line just inside the park line — and a safety floor guarantees an enemy that could still sense the player is never parked.” → „Koncentryczne pierścienie wokół gracza, z legendą obok. Najbardziej wewnętrzny krąg jest aktualizowany co klatkę, a jego promień to zasięg zmysłów plus margines. Środkowy pas aktualizuje się co interwał, z losowym rozrzutem. Zewnętrzny pas jest uśpiony i zwalnia zapamiętaną ścieżkę. Dwa bliskie okręgi na granicy uśpienia oznaczają histerezę — linia wybudzenia leży tuż wewnątrz linii uśpienia — a dolny próg bezpieczeństwa gwarantuje, że przeciwnik, który mógłby jeszcze wyczuć gracza, nigdy nie zostanie uśpiony.”
- System 4, diagram stref, wersja wąska (`cs-rings-tall`): tekst jak wyżej, tylko „with a legend below them” → „z legendą pod spodem”.
